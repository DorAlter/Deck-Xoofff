...
#ifdef UT_OUTPUT
void writeTestDeckTink_One(FILE *f)
{
    unsigned char checksum[checksumByteSize];
    unsigned int offset;

    printf("Writing Deck-Tink ");
    performTestDeckTink(checksum);
    fprintf(f, "    selfTestDeck-Tink(\"");
    for(offset=0; offset<checksumByteSize; offset++)
        fprintf(f, "\\x%02x", checksum[offset]);
    fprintf(f, "\");\n");
    printf("\n");
...
void writeTestDeckTink(const char *filename)
{
    FILE *f = fopen(filename, "w");
    assert(f != NULL);
    writeTestDeckTink_One(f);
    fclose(f);
}
#endif
...
/*    printXooTestVectors(); */
    ...
    selfTestDeckTink((const unsigned char *)"\xb1\xa1\xc\x2\x5b\xca\xe8\xb5\x18\x42\x75\x66\x75\x9\xf8\x7b");
#endif
}
