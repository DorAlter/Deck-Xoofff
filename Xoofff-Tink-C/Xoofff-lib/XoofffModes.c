...
static int Deck_Tink_AddToContext(Xoofff_Instance *xp, const BitSequence *data, BitLength dataBitLen, unsigned char appendix)
{
    BitSequence lastByte[1];

    if (Xoofff_Compress(xp, data, dataBitLen & ~7, Xoofff_FlagNone) != 0) /* Do all except last byte if incomplete */
        return 1;

    data += dataBitLen >> 3; /* move pointer to last incomplete byte (if no incomplete last byte, it will point beyond the buffer, but pointer won't be dereferenced) */
    dataBitLen &= 7; /* dataBitLen is now number of bits in last possible incomplete byte */
    if (dataBitLen == 0) {
        lastByte[0] = (BitSequence)(appendix);
        dataBitLen = 1;
    }
    else {
        lastByte[0] = (BitSequence)(*data | (appendix << dataBitLen));
        dataBitLen += 1;
    }
    
    return Xoofff_Compress(xp, lastByte, dataBitLen, Xoofff_FlagLastPart);
}


int Deck_Tink_Initialize(Deck_Tink_Instance *xp, const BitSequence *Key, BitLength KeyBitLen, const BitSequence *Nonce, BitLength NonceBitLen)
{
    xp->finish = 0;
    Xoofff_MaskDerivation(&xp->xoofff, Key, KeyBitLen);
    
    return Xoofff_Compress(&xp->xoofff, Nonce, NonceBitLen, Xoofff_FlagInit | Xoofff_FlagLastPart);
}

int Deck_Tink_Wrap(Deck_Tink_Instance *xp, const BitSequence *plaintext, BitSequence *ciphertext, BitLength dataBitLen, 
                        unsigned char *tag, unsigned int done, const BitSequence *counter)
{

   /* if finish = 1 then error finished instance*/
   if (xp->finish != 0)
        return 1;

    Xoofff_Instance encrypt;
    memcpy(&encrypt, &xp->xoofff, sizeof(xp->xoofff));


    /* if |P| > 0 then*/
    if (dataBitLen !=0){
        /* X = P + Q'.F_k(counter||0)*/
        if(Deck_Tink_AddToContext(&encrypt, counter, 64, 0 ) != 0) 
            return 1;

        if ( Xoofff_Expand(&encrypt, ciphertext, dataBitLen, Xoofff_FlagLastPart) != 0)
            return 1;
        
        Xoofff_AddIs(ciphertext, plaintext, dataBitLen);

        /* T = 0^t + Q'.F_k(X||1)*/
        if(Deck_Tink_AddToContext(&encrypt, ciphertext, dataBitLen, 1 ) != 0) 
            return 1;

        if ( Xoofff_Expand(&encrypt, tag, Deck_Tink_TagLength * 8, Xoofff_FlagLastPart) != 0)
            return 1;
    } else {
        /* T = 0^t + Q'.F_k(counter||1)*/
        if(Deck_Tink_AddToContext(&encrypt, counter, 64, 1 ) != 0) 
            return 1;
...
int Deck_Tink_Unwrap(Deck_Tink_Instance *xp, const BitSequence *ciphertext, BitSequence *plaintext, BitLength dataBitLen, 
                            const unsigned char *tag, unsigned int done, const BitSequence *counter)
{
    unsigned char tagPrime[Deck_Tink_TagLength];

    /* if finish = 1 then error finished instance*/
   if (xp->finish != 0)
        return 1;

    /* if |C| < t then error finished instance*/
    if (dataBitLen < Deck_Tink_TagLength * 8)
        return 1;

    /* Q' = Q */
    Xoofff_Instance decrypt;
    memcpy(&decrypt, &xp->xoofff, sizeof(xp->xoofff));
    Xoofff_Instance decrypt2;
    
    /* if |C| > 0 then*/
    if (dataBitLen > Deck_Tink_TagLength * 8){
        /* Q'.F_k(counter||0)*/
        if(Deck_Tink_AddToContext(&decrypt, counter, 64, 0 ) != 0) 
            return 1;
        
        /* Q'' = Q'*/
        //decrypt2 =  decrypt->xoofff;
        memcpy(&decrypt2, &decrypt, sizeof(decrypt));

        /* T' = 0^t + Q'.F_k(X||1)*/
        if( Deck_Tink_AddToContext(&decrypt, ciphertext, dataBitLen-Deck_Tink_TagLength*8, 1 ) != 0) 
            return 1;

        if ( Xoofff_Expand(&decrypt, tagPrime, Deck_Tink_TagLength * 8, Xoofff_FlagLastPart) != 0)
            return 1;
        
    } else {
        /* T' = 0^t + Q'.F_k(X||1)*/
        if(Deck_Tink_AddToContext(&decrypt, counter, 64, 1 ) != 0) 
            return 1;
    ...
    /* T' != T then*/
    if ( memcmp( tagPrime, tag, Deck_Tink_TagLength) != 0) {
        return 1;
    }

     /* if |C| > 0 then*/
    if (dataBitLen > Deck_Tink_TagLength * 8){
        if ( Xoofff_Expand(&decrypt2, plaintext, dataBitLen-Deck_Tink_TagLength*8, Xoofff_FlagLastPart) != 0)
            return 1;
        Xoofff_AddIs(plaintext, ciphertext, dataBitLen-Deck_Tink_TagLength*8);
    }
